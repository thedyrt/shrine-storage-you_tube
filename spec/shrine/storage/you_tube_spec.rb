...
describe Shrine::Storage::YouTube, :vcr do
  ...
  describe '#upload' do
    ...
    let(:upload_result) { youtube_storage.upload(video, 'original_id'.dup, shrine_metadata: metadata, **passed_upload_options) }
    ...
    it 'uploads to the original storage with the YouTube ID' do
      ...
      expect(original_storage).to have_received(:upload).with(video, youtube_id, shrine_metadata: {})
    end
  end
